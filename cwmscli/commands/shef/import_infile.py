...
Usage
-----
    python shef_in_to_cwms_group.py                       \\
        --in-file   exportShef_1-10-2.in                  \\
        --group-name "CEMVP Lock-Dam LD1-10"              \\
        --office    MVP                                   \\
        --api-root  https://your-cwms-host/cwms-data      \\
        [--api-key  YOUR_API_KEY]                         \\
        [--token    KEYCLOAK_TOKEN]                       \\
        [--category "SHEF Export"]                        \\
        [--fail-if-exists]                                \\
        [--dry-run]                                       \\
        [--output-json payload.json]
"""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Parse an exportShef .in file and create/update a CWMS "
            'timeseries group (category="SHEF Export") where each '
            "TSID's alias-id carries the SHEF encoding."
        ),
    ...
    p.add_argument(
        ...
        help=(
            ...
            "or the CDA must support auto-creation when the group is POSTed."
        ),
    )
    p.add_argument(
        "--fail-if-exists",
...
def main(argv: Optional[list] = None) -> int:
    ...
    if not args.in_file.exists():
        log.error("File not found: %s", args.in_file)
        return 1

    if not args.dry_run and not args.api_root:
        log.error("--api-root is required unless --dry-run is specified.")
        return 1

    # ---- Parse .in file --------------------------------------------------
    log.info("Parsing: %s", args.in_file)
    header, entries = parse_in_file(args.in_file, csv_path=args.shef_params)
    ...
    if not entries:
        log.error(
            "No timeseries entries found in %s.\n"
            "  Supported formats:\n"
            "    TSID  SHEF_LOC  PE  DUR  SEND       (whitespace columns)\n"
            "    TSID|SHEF_LOC|PE|DUR|SEND           (pipe-delimited)\n"
            "    TSID,SHEF_LOC,PE,DUR,SEND           (comma-delimited)\n"
            "    TSID = SHEF_LOC.PE.DUR.SEND         (dot-joined alias)\n"
            "  Tip: use --verbose to see debug output.",
            args.in_file,
        )
        return 1
    ...
    if args.output_json:
        args.output_json.write_text(json.dumps(group_json, indent=2))
        log.info("JSON payload written to: %s", args.output_json)
    ...
    # ---- Connect to CWMS Data API ----------------------------------------
    log.info("Connecting to API: %s", args.api_root)
    cwms_api.init_session(
