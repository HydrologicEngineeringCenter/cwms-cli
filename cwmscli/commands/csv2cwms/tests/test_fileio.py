...
def test_load_csv_ignores_hash_rows():
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".csv", delete=False, encoding="utf-8"
    ) as commented:
        commented.write("# generated by upstream system\n")
        commented.write("Time,Value\n")
        commented.write("# skipped row,do not parse\n")
        commented.write("2025-01-01 00:15,42\n")
        commented_path = commented.name

    try:
        result = load_csv(commented_path)
        assert result == [["Time", "Value"], ["2025-01-01 00:15", "42"]]
    finally:
